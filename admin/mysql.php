...
if (!stristr($_SERVER['HTTP_REFERER'],$_SERVER['HTTP_HOST']) && !stristr($_SERVER['HTTP_REFERER'],'admincp.php')){
	echo '不允许外部链接';
	die();
}
...
$db_host=$dbhost;    //数据库服务器
$db_username=$dbuser;     //数据库用户名
$db_password=$dbpw;         //数据库密码
$db_dbname=$dbname;          //选择的数据库
...
//各页相同的页面尾
function ffooter(){
	echo "<div id='pageendTag'></div></form></center>".showmywin_script()."</body></html>";
}
...
function fbutton($type="submit",$name="Submit",$value="确定",$script="",$return=0){
	$str="<input type='$type' name='$name' value='$value' class='btn' $script> ";
...
if($_GET["action"]=="readme"){
	...
	echo tabledata(implode('',file("../images/admincp/admin_mysql_back_read.htm")));
	echo tableend();
	ffooter();
	exit;
}
...
if (!isset($_POST['action'])){
	...
	while ($currow=mysql_fetch_array($query)){
		echo tabledata("<center><input name='fsqltable[{$currow[Name]}]' id='fsqltable_$currow[Name]' type='checkbox' value='".($currow[Data_length]+$currow[Index_length]).",".$currow[Avg_row_length]."' checked onclick='getsize()' style='border:0px;'></center>|<label for='fsqltable_$currow[Name]'>$currow[Name]</label>|$currow[Comment]|$currow[Rows]|".num_bitunit($currow[Data_length]+$currow[Index_length])."");
	}
...
if($_POST['action']=="selecttype"){
	...
	echo tabledata("导出方式：|<br>
					<input name='back_type' value='download' style='border:0px;' type='radio' $downloadck>生成单个文件并下载 (备份的数据量较大时不建议使用)<br>
					<input name='back_type' value='partsave' style='border:0px;' type='radio' $partsaveck>分为多个文件并保存在服务器 <br><br>");
	echo tableend();
	echo "
	<script language='JavaScript'>
	function confirmit(){
...
if($_POST['action']=="selectoption"){
	...
	if($_POST['back_type']=="download"){//单文件下载选项
		...
		echo tabledata("生成文件格式：|<input name='extension' value='php' type='radio' checked style='border:0px;'>.php <input name='extension' value='zip' type='radio'  style='border:0px;'>.zip <input name='extension' value='gz' type='radio'  style='border:0px;'>.gz");
		echo tableend();
...
if($_POST['action']=="databackup"){
	...
	if($_POST[back_type]=="partsave"): ////////////////////////   Save Data ////////////////////////////
		...
		if($_POST[extension]=="zip"){
			include("./admin/class/zipclass.php");
		...
		if($_POST[page]==0){
			...
			if(isset($fsqlzip)){
				...
				*/
				...
				$fsqlzip->addfile(implode('',file("../images/admincp/faisunsqllogo.gif")),"{$_POST[filename]}_faisunsqllogo.gif","$fsqlzip->gzfilename.tmp");
			}else{
				if(!@copy("../images/admincp/faisunsqllogo.gif","$_POST[dir]/{$_POST[filename]}_faisunsqllogo.gif")){
		...
		//结束最后文件
		$data="echo '<center><BR><BR><BR><BR>完成。所有数据都已经导入数据库中。</center>	<br><br><font color=red><b><a href=\'?Act=readme' target=\'_blank\'>请阅读《说明文档》</a></b> </font>'; exit; ?".">";
		...
		if($_GET["framename"]=="topframe"&&$_GET["action"]=="topframe"){
			echo "<html><body><center><img src=\''.$_POST[filename].'_faisunsqllogo.gif\' border=0 width=300 height=71></center></body></html>";
			exit;
		}
		?'.'><html><head>
		<meta http-equiv="Content-Type" content="text/html; charset=gb2312">
